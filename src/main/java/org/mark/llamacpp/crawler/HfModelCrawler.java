...
public final class HfModelCrawler {
  ...
  public static GGUFCrawlResult crawlGGUFFiles(String modelUrlOrRepoId, int timeoutSeconds) throws IOException, InterruptedException {
    if (modelUrlOrRepoId == null || modelUrlOrRepoId.isBlank()) throw new IllegalArgumentException("modelUrlOrRepoId 不能为空");
    String baseUrl = determineBaseUrl(modelUrlOrRepoId);
    String repoId = normalizeToRepoId(modelUrlOrRepoId);
    if (repoId == null) throw new IllegalArgumentException("无法从输入解析出 repoId： " + modelUrlOrRepoId);
  ...
  public static ModelSearchResult searchModels(String query, int limit, int timeoutSeconds) throws IOException, InterruptedException {
    return searchModels(query, limit, timeoutSeconds, 0, 0);
  }

  public static ModelSearchResult searchModels(String query, int limit, int timeoutSeconds, int startPage, int maxPages) throws IOException, InterruptedException {
    ...
    for (int fetchedPages = 0; hits.size() < safeLimit && fetchedPages < safeMaxPages && consecutiveNoNew < 2; fetchedPages++) {
  ...
  private static JsonObject fetchModelInfo(String baseUrl, String repoId, int timeoutSeconds) throws IOException, InterruptedException {
  ...
  private static JsonArray fetchRepoTree(String baseUrl, String repoId, String revision, int timeoutSeconds) throws IOException, InterruptedException {
    HttpClient client = HttpClient.newBuilder()
        .connectTimeout(Duration.ofSeconds(timeoutSeconds))
        .followRedirects(HttpClient.Redirect.NORMAL)
        .build();
    ...
    if (response.statusCode() < 200 || response.statusCode() >= 300) {
      ...
      HttpResponse<String> fallbackResponse = client.send(fallbackBuilder.GET().build(), HttpResponse.BodyHandlers.ofString(StandardCharsets.UTF_8));
      if (fallbackResponse.statusCode() >= 200 && fallbackResponse.statusCode() < 300) {
        ...
        if (fallbackRoot.isJsonArray()) return fallbackRoot.getAsJsonArray();
        return new JsonArray();
      }
      throw new IOException("请求失败: HTTP " + response.statusCode() + " " + uri + "\n" + bodyPreview);
    }
    JsonElement root = JsonParser.parseString(response.body());
    if (!root.isJsonArray()) return new JsonArray();
    return root.getAsJsonArray();
  }

  private record SearchApiPage(JsonArray models, String nextCursor) {}

  private static SearchApiPage fetchModelsSearchApiPage(String baseUrl, String query, String cursor, int limit, int timeoutSeconds) throws IOException, InterruptedException {
    HttpClient client = HttpClient.newBuilder()
        .connectTimeout(Duration.ofSeconds(timeoutSeconds))
        .followRedirects(HttpClient.Redirect.NORMAL)
        .build();
    ...
    url.append(baseUrl).append("/api/models?search=").append(q).append("&filter=gguf&full=true&limit=").append(Math.max(1, limit));
    if (cursor != null && !cursor.isBlank()) {
  ...
  private static void collectModelSearchHits(JsonArray models, int limit, Set<String> seen, List<ModelSearchHit> hits, String baseUrl) {
    if (models == null || models.isEmpty()) return;
    int safeLimit = Math.max(1, limit);
    if (hits.size() >= safeLimit) return;
    for (JsonElement el : models) {
      if (el == null || !el.isJsonObject()) continue;
      JsonObject obj = el.getAsJsonObject();
      String repoId = getString(obj, "modelId");
      if (repoId == null || repoId.isBlank()) repoId = getString(obj, "id");
      ...
      hits.add(new ModelSearchHit(repoId, modelUrl, pipelineTag, lastModified, libraryName, downloads, likes, parameters));
  ...
  private static String normalizeToRepoId(String input) {
    ...
    if (trimmed.startsWith("http://") || trimmed.startsWith("https://")) {
      try {
        ...
        if (!isSupportedHost(uri.getHost())) return null;
  ...
  private static List<GGUFFileInfo> extractGGUFFiles(String baseUrl, String repoId, String revision, JsonArray tree) {
    ...
    for (JsonElement el : tree) {
      ...
      if (!path.toLowerCase(Locale.ROOT).endsWith(".gguf")) continue;
  ...
  private static String determineBaseUrl(String input) {
    if (input == null) return getDefaultBaseUrl();
    String trimmed = input.trim();
    if (trimmed.startsWith("http://") || trimmed.startsWith("https://")) {
      try {
        URI uri = URI.create(trimmed);
        if (uri.getHost() != null && uri.getHost().trim().equalsIgnoreCase("hf-mirror.com")) return HF_MIRROR_BASE;
      } catch (Exception ignored) {
      }
    }
  ...
  private static String getDefaultBaseUrl() {
    ...
    if (env != null && !env.isBlank()) {
      ...
      if (v.equalsIgnoreCase(HF_BASE) || v.equalsIgnoreCase(HF_MIRROR_BASE)) return v;
    }
    return HF_BASE;
  }
}
